...
class ProductDaoFile {
  ...
  async addProducts(nuevoProducto) {
    try {
  ...
  async getProductById(pid) {
    try {
      ...
      let product = productosJSON.find((prod) => prod.id === pid);
      //console.log(product);
      if (!product) return `getProductById: Not found. No existe un producto con el id: ${pid}`;
      return product;
    } catch (error) {
      console.log(`getProducts DAO File: ${error}`);
    }
  }
  ...
  async deleteProduct(pid) {
    try {
      ...
      let productosJSON = JSON.parse(productosString);
      this.products = productosJSON;
      const delId = this.products.find((producto) => producto.id === pid);
      console.log(`El producto ${delId.title} ha sido eliminado`);
      //Busca indice del producto y lo elimina
      const deleteIndex = this.products.findIndex((producto) => producto.id == pid);
